...
export async function handleImageEditCore({
  ...
  if (!adapter?.supportsEdit) {
    return createErrorResult(HTTP_STATUS.BAD_REQUEST, `Provider '${provider}' does not support image editing`);
  }

  const body = { image, prompt: prompt.trim() };
  for (const field of ["size", "response_format", "seed", "steps", "cfg_scale", "negative_prompt", "text_mode"]) {
  ...
  try {
    ...
    return createErrorResult(HTTP_STATUS.BAD_REQUEST, error.message || `Invalid ${provider} image edit request`);
  }

  log?.debug?.("IMAGE", `${provider.toUpperCase()} | ${model} | edit prompt="${body.prompt.slice(0, 50)}..."`);
  ...
  if (!providerResponse.ok) {
    const { statusCode, message } = await parseUpstreamError(providerResponse);
    return createErrorResult(statusCode, formatProviderError(new Error(message), provider, model, statusCode));
  }
  ...
  try {
    ...
    return createErrorResult(HTTP_STATUS.BAD_GATEWAY, error.message || `Invalid response from ${provider}`);
  }
