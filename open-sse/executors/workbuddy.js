...
//   2. The first message MUST be a system prompt or the call fails with
//      code 11128 "first message is not system prompt".
//   3. Non-stream requests are rejected (code 11101); registry sets forceStream.
...
export class WorkbuddyExecutor extends DefaultExecutor {
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    if (first?.role === "system") return transformed;
    return {
      ...transformed,
      messages: [{ role: "system", content: "You are a helpful assistant." }, ...transformed.messages],
    };
  }
