...
import { NOVITA_PRICING_SNAPSHOT } from "./novita.js";
...
export const MODEL_PRICING = {
  ...
  "gemini-3.6-flash-tiered":      { input: 0.50,  output: 3.00,  cached: 0.03,  reasoning: 4.50,   cache_creation: 0.50  },
  "gemini-3.1-flash-lite":        { input: 0.25,  output: 1.50,  cached: 0.02,  reasoning: 2.25,   cache_creation: 0.25  },
  "gemini-3.1-flash-image":       { input: 0.50,  output: 3.00,  cached: 0.03,  reasoning: 4.50,   cache_creation: 0.50  },
  ...
  "gemini-2.5-flash-thinking":    { input: 0.30,  output: 2.50,  cached: 0.03,  reasoning: 3.75,   cache_creation: 0.30  },
...
export const PROVIDER_PRICING = {
  ...
  // Novita — exact provider IDs must win over canonical/pattern pricing. The
  // values include source metadata because Novita's catalog, promotions and
  // time-limited free offers change independently of model names.
  novita: NOVITA_PRICING_SNAPSHOT,
};
...
function computePricingForModel(provider, model) {
  // WorkBuddy bills in its own native credit unit. Its xN coefficients are
  // not USD/token rates, so never fall through to canonical or pattern USD
  // pricing for a WorkBuddy request. Novita's prices are provider-specific
  // snapshots too: an unlisted Novita model must not inherit a similarly named
  // price from another provider or a generic pattern.
  if (provider === "workbuddy" || provider === "novita") {
