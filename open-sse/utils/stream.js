...
export function createSSEStream(options = {}) {
  ...
  let buffer = "";
  let usage = null;
  // True while `usage` holds a content-based ESTIMATE rather than a
  // provider-reported value. Providers that do report usage send it in a
  // terminal chunk that arrives AFTER the finish chunk, so the finish-chunk
  // estimate must not win: mergeUsage() max-merges, and the estimate carries a
  // +2000 token buffer that would silently overwrite the real prompt count.
  ...
  // Request logging is intentionally decoupled from the stream hot path. The
  // logger returns a Promise only when its bounded queue needs backpressure;
  // normal append calls are enqueue-only and return immediately.
  const pendingLoggerWrites = [];
  let loggerClosed = false;

  function appendLogger(method, value) {
    if (!reqLogger || typeof reqLogger[method] !== "function") return;
    try {
      const result = reqLogger[method](value);
      if (result && typeof result.then === "function") pendingLoggerWrites.push(result);
    } catch {
      // Logging must never turn a provider stream into a failed request.
    }
  }

  async function drainLoggerBackpressure() {
    if (!pendingLoggerWrites.length) return;
    const writes = pendingLoggerWrites.splice(0, pendingLoggerWrites.length);
  ...
  const transformStream = new TransformStream({
    async transform(chunk, controller) {
      ...
      buffer += text;
      appendLogger("appendProviderChunk", text);
      ...
      for (const line of lines) {
        ...
        if (mode === STREAM_MODE.PASSTHROUGH) {
          ...
          if (trimmed.startsWith("data:") && trimmed.slice(5).trim() !== "[DONE]") {
            try {
              ...
              if (extracted) {
                // Provider-reported usage is authoritative — replace an earlier
                // estimate instead of max-merging with it.
                usage = usageEstimated ? extracted : mergeUsage(usage, extracted);
        ...
        if (parsed && parsed.done && targetFormat !== FORMATS.OLLAMA) {
          ...
          if (keepsOpenAIResponsesFormat && !openAIResponsesTerminalSeen) {
            ...
            appendLogger("appendConvertedChunk", failedOutput);
          ...
          if (keepsOpenAIResponsesFormat && !streamDoneSent) {
            ...
            appendLogger("appendConvertedChunk", doneOutput);
        ...
        if (translated?._openaiIntermediate) {
          for (const item of translated._openaiIntermediate) {
            ...
            appendLogger("appendOpenAIChunk", openaiOutput);
          }
        }
    ...
    async flush(controller) {
      ...
      try {
        ...
        if (mode === STREAM_MODE.PASSTHROUGH) {
          ...
          // Gemini-family clients (Antigravity, Gemini) reject this sentinel with 400 syntax errors.
          const isGeminiFamily = provider === "antigravity";
