...
//
// `stream_options` is added by DefaultExecutor.ensureStreamUsageRequested()
// *before* this runs, precisely so an upstream that rejects the field can be
// opted out here with a one-line rule instead of a branch in the executor:
//
//   { provider: "some-provider", drop: ["stream_options"] }
//
// No provider needs that today — OpenRouter/Kilo Code/Groq/DeepSeek all accept
// it. To disable the usage request for every provider at once (no rebuild
// needed) set SWITCH_ROUTER_STREAM_USAGE=off instead.
const STRIP_RULES = [
