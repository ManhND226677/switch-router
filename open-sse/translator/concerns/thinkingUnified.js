...
function applyFormat(fmt, body, cfg, caps) {
  ...
  switch (fmt) {
    case "openai": {
      ...
      // "auto" is the router's sentinel for "provider decides", not a member of
      // OpenAI's reasoning_effort enum. Forwarding the literal string makes strict
      // upstreams reject the whole request (e.g. WorkBuddy 400 code 11150
      // invalid_reasoning_effort), so omit the field — same semantics, since the
      // model then runs at its default effort.
      if (level === "auto") break;
