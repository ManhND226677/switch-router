...
- **Provider `unstoppable`: thay 4 model fallback bằng catalog thật (25 model) + bật quota AI Credits + bắt buộc stream.** Sau khi login chạy được, phát hiện catalog model lấy từ `GET /api/v1/desktop-app/ai-gateway/models?catalogVersion=5` → `{version, models:[{id, gatewayId, label, family, contextWindow, recommendedHarness, ...}]}` (25 model, 6 family: anthropic/openai/deepseek/moonshot/minimax/zai). Ba điều chỉnh: (1) **Model**: `gatewayId` dạng `<family>/<model>` mới là thứ proxy route, còn `id` là dạng ngắn người dùng gõ sau `udc/` — nên mỗi entry khai báo `upstreamModelId` (cơ chế sẵn có ở `getModelUpstreamId`), ví dụ `udc/claude-sonnet-5` → upstream `anthropic/claude-sonnet-5`. (2) **Stream**: proxy từ chối request không stream (`{"error":"LLM proxy requests must set stream: true."}`, verify live) → thêm `forceStream: true`. (3) **Quota**: thêm handler `open-sse/services/usage/unstoppable.js` đọc `GET /api/v1/ai-credits/balance` (`credits` = số dư còn lại, `dailyDripCapCredits` = trần drip theo ngày) + `GET /api/v1/ai-credits/subscription` (plan/renewsAt/active), đăng ký trong `USAGE_HANDLERS` + `features.usageApikey: true` (endpoint nhận cùng cskToken nên connection dán API key cũng có quota). Ngữ nghĩa `credits` vs `dailyDripCapCredits` lấy từ chính app desktop (`welcomeCreditsRemaining`): chỉ vẽ thanh % khi cap thực sự áp dụng — subscription trả phí đang active hoặc có `orgPool` thì chỉ hiện số dư, không bịa mẫu số. Bỏ `refreshLeadMs` (không có refresh flow: cskToken là API key dài hạn, token response không có expiry) và bỏ `refreshable` trong `testUtils`. Verify live bằng guest token: `POST .../llm-proxy/anthropic/v1/messages` với `{model:"openai/gpt-5.6-luna", stream:true}` → HTTP 200 + SSE Anthropic hợp lệ (text "OK"), xác nhận **một transport phục vụ được cả catalog** chứ không chỉ nhánh `anthropic/*`. Test `unstoppable-provider.test.js` nâng lên 14 case (catalog + upstreamModelId, forceStream, usage endpoints, balance→quota row, paid-seat không vẽ bar, 401 không trả số dư giả, thiếu credential).
...
- **Login Unstoppable "thành công" nhưng connection không có credential → mọi request trả 401 "Invalid API key".** `mapTokens` đọc `tokens.access_token` theo chuẩn oauth2, nhưng endpoint `POST /api/v1/desktop-app/desktop-auth/token` trả **shape camelCase riêng của app desktop**: credential nằm ở field **`token`** (chính là cskToken = API key của account), kèm `userApiKey`/`account`/`capabilities`/`organizations`. Hệ quả: `accessToken` = `undefined` → `createProviderConnection` lưu connection chỉ có `testStatus` (DB thật: `{"testStatus":"active"}`, `hasCredentials:false`) → request kế tiếp gửi `Authorization: Bearer undefined` → 401. Bằng chứng đối chiếu: hàm map trong `app.asar` (`hwr`) trả `token: fc(e,"token")` và luồng guest (`Qyr`) dùng thẳng `apiKey: a.token`; verify live `POST .../desktop-auth/guest` cũng trả `{"token":"csk_…","userApiKey":{…},"account":{…}}`. Fix: `accessToken: tokens.token` + lấy thêm `email` từ `tokens.account.email` để đặt tên connection. **Lưu ý:** connection đã tạo bằng bản cũ vẫn rỗng credential — cần xoá và login lại.
