...
// Process-level liveness log. Started here — not inside the Next.js app — so it
// runs at boot regardless of traffic, which is what makes a gap in the log
// meaningful. See gateway-heartbeat.js for the reasoning.
const { createGatewayHeartbeat } = require("./gateway-heartbeat.js");
const heartbeat = createGatewayHeartbeat();
heartbeat.start();
process.on("exit", () => heartbeat.stop("process exiting"));
