...
// ───────────────────────────────────────────────────────────────────────────
// P2 — 0.10.1 retired 4 providers; their leftover connections must go dormant
// ───────────────────────────────────────────────────────────────────────────
describe("migration 005: retired-provider connections", () => {
  it("deactivates retired providers only, keeps credentials, and is re-runnable", async () => {
    const mod = await import("@/lib/db/migrations/005-retire-removed-providers.js");
    const m005 = mod.default;
    ...
    const node = await db.createProviderNode({
      id: "openai-compatible-chat-retire-test", type: "openai-compatible",
      name: "Retire test", prefix: "rt", apiType: "chat", baseUrl: "https://example.test/v1",
    });
    adapter.run(
      `INSERT INTO providerConnections (id, provider, authType, name, priority, isActive, data, createdAt, updatedAt)
       VALUES (?, ?, 'apikey', ?, 1, 1, ?, ?, ?)`,
      ["conn-retired", retiredId, "Old retired connection", JSON.stringify({ apiKey: "keep-me-please" }), "2026-01-01T00:00:00.000Z", "2026-01-01T00:00:00.000Z"]
    );
    adapter.run(
      `INSERT INTO providerConnections (id, provider, authType, name, priority, isActive, data, createdAt, updatedAt)
       VALUES (?, ?, 'apikey', ?, 1, 1, ?, ?, ?)`,
      ["conn-live", node.id, "Still valid", JSON.stringify({ apiKey: "x" }), "2026-01-01T00:00:00.000Z", "2026-01-01T00:00:00.000Z"]
    );

    m005.up(adapter);

    const readRow = (id) => adapter.get(`SELECT isActive, data FROM providerConnections WHERE id = ?`, [id]);
    expect(readRow("conn-retired").isActive).toBe(0);
    expect(readRow("conn-live").isActive).toBe(1);
    // Deactivate, never delete: the stored credential must survive for recovery.
    expect(JSON.parse(readRow("conn-retired").data).apiKey).toBe("keep-me-please");

    const stampedAt = adapter.get(`SELECT updatedAt FROM providerConnections WHERE id = 'conn-retired'`).updatedAt;
    m005.up(adapter);
    expect(adapter.get(`SELECT isActive, updatedAt FROM providerConnections WHERE id = 'conn-retired'`).isActive).toBe(0);
    expect(adapter.get(`SELECT updatedAt FROM providerConnections WHERE id = 'conn-retired'`).updatedAt).toBe(stampedAt);
  });
});
...
describe("schema declaration covers migrated columns", () => {
  it("apiKeys policy columns from migration 004 are declared in TABLES", async () => {
    ...
    for (const column of ["allowedModels", "monthlyBudgetUsd", "rateLimitRpm", "expiresAt", "lastUsedAt", "isHidden"]) {
      expect(TABLES.apiKeys.columns, `${column} must stay declared so syncSchemaFromTables can repair it`)
