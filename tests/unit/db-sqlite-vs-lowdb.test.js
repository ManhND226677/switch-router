...
describe("DB SQLite layer — public API parity", () => {
  it("settings: get → defaults; update → merge", async () => {
    ...
    // Dashboard auth is intentionally disabled for this local build (settingsRepo.js)
    expect(s.requireLogin).toBe(false);

    const updated = await sqliteDb.updateSettings({ customField: "x" });
    expect(updated.customField).toBe("x");
    expect(updated.requireLogin).toBe(false); // default preserved
  ...
  it("apiKeys: create/get/validate/delete", async () => {
    ...
    const hidden = await sqliteDb.createApiKey("model-test-hidden", "machine-abc", { isHidden: true });
    expect(hidden.isHidden).toBe(true);
    expect((await sqliteDb.getApiKeys({ includeHidden: false })).some((x) => x.id === hidden.id)).toBe(false);
    expect((await sqliteDb.getApiKeys({ includeHidden: true })).some((x) => x.id === hidden.id)).toBe(true);
    expect(await sqliteDb.getApiKeyById(hidden.id, { includeHidden: false })).toBeNull();
    expect((await sqliteDb.getApiKeyById(hidden.id, { includeHidden: true })).isHidden).toBe(true);

    const deleted = await sqliteDb.deleteApiKey(k.id);
    expect(deleted).toBe(true);
    expect(await sqliteDb.getApiKeyById(k.id)).toBeNull();
    await sqliteDb.deleteApiKey(hidden.id);
  });
