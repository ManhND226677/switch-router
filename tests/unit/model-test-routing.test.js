...
describe("model test route kind routing", () => {
  ...
  it("model probes prefer the hidden internal test key over a restricted visible key", async () => {
    mocks.getApiKeys.mockResolvedValue([
      { key: "sk-visible", isActive: true, isHidden: false },
      { key: "sk-model-test", isActive: true, isHidden: true },
    ]);

    const { getInternalHeaders } = await import("../../src/app/api/models/test/ping.js");
    const headers = await getInternalHeaders();

    expect(headers.Authorization).toBe("Bearer sk-model-test");
  });
