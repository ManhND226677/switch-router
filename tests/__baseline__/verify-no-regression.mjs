...
const nowFails = r.testResults.flatMap(f =>
  f.assertionResults.filter(a => a.status === "failed")
    .map(a => {
      // jest reports absolute paths ("/app/..."); vitest reports absolute or relative paths
      const normalized = f.name.replace(/\\/g, "/");
      const marker = normalized.indexOf("/app/");
      const testsMarker = normalized.indexOf("tests/");
      const file = marker >= 0 ? normalized.slice(marker + 5)
        : testsMarker >= 0 ? normalized.slice(testsMarker)
        : normalized.replace(/^\.?\//, "");
      const qualified = file.startsWith("tests/") ? file : `tests/${file}`;
      return qualified + " :: " + a.fullName;
    })
);
