...
const PROVIDERS = {
  ...
  // Unstoppable Code (Canopy Cloud) — authorization_code + PKCE against
  // app.unstoppable.ai. Both the authorize URL and the token exchange use
  // non-standard shapes: the authorize URL carries client_name/client_device_id/
  // client_version/client_hostname instead of client_id, and the token endpoint
  // takes a JSON body with camelCase fields (grantType, codeVerifier, redirectUri,
  // clientName, ...) rather than the usual form-encoded oauth2 params.
  unstoppable: {
    config: UNSTOPPABLE_CONFIG,
    flowType: "authorization_code_pkce",
    // Resolve the client_identity fields the Unstoppable desktop-auth flow expects.
    // The official app sends a persistent device id, its own version, and the OS
    // hostname; we approximate with a per-login UUID + this gateway's version.
    prepareConfig: (config) => ({
      ...config,
      clientDeviceId: randomUUID(),
      clientVersion: "switch-router/0.11.0",
      clientHostname: hostname() || "localhost",
    }),
    buildAuthUrl: (config, redirectUri, state, codeChallenge) => {
      const params = new URLSearchParams({
        state,
        ...
        client_device_id: config.clientDeviceId,
        client_version: config.clientVersion,
        client_hostname: config.clientHostname,
      });
      return `${config.authorizeUrl}?${params.toString()}`;
    },
