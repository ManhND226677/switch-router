// 005: Dọn dữ liệu còn sót sau khi gỡ 4 provider khỏi registry ở 0.10.1
// (qoder, gemini-cli, gemini, ollama-local).
//
...
// SCHEMA_VERSION đã bump lên 5 kèm migration này (data-repair-regression bắt
// latestVersion() === SCHEMA_VERSION), nên migrate.js cũng tự chụp một bản
// backup trước khi áp dụng.
export const RETIRED_PROVIDERS = ["qoder", "gemini-cli", "gemini", "ollama-local"];
...
const migration = {
  ...
  up(db) {
    for (const provider of RETIRED_PROVIDERS) {
      ...
      if (deactivated > 0) {
        console.log(`[DB][migrate] 005: tắt ${deactivated} connection của provider đã gỡ "${provider}" (giữ nguyên credential, bật lại thủ công nếu cần)`);
      }
    }
  },
};
