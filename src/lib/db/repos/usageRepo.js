...
async function calculateCost(provider, model, tokens) {
  ...
  // WorkBuddy's native xN coefficients are credits, not USD/token prices.
  // Keep native credit accounting separate even if a stale/manual USD override
  // happens to exist in the local pricing store.
  if (provider === "workbuddy") return 0;
  try {
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        // never surface as a rejection to callers. Prefixed so a dropped usage
        // row is greppable in the server log instead of vanishing silently.
        console.error("[usageRepo] Failed to save usage stats:", e);
      }
      item.resolve();
    }
  } catch (e) {
...
/**
 * Aggregate gateway-observed usage for one provider connection.
 *
 * This intentionally queries by connectionId (and optionally provider) rather
 * than using the daily provider rollup. A WorkBuddy/Novita account's local
 * observed history must never be mixed with another account, and WorkBuddy's
 * native credits cannot be inferred from these token counters.
 */
export async function getObservedUsageForConnection(connectionId, provider = null) {
  const empty = {
    scope: "local",
    period: "all",
    requests: 0,
    ...
    cachedTokens: 0,
    lastUsed: null,
    costUsd: provider === "workbuddy" ? null : 0,
    pricingStatus: provider === "workbuddy" ? "not_applicable" : "no_data",
  };
  ...
  const rows = db.all(
    `SELECT timestamp, provider, model, promptTokens, completionTokens, cost, tokens
     FROM usageHistory WHERE ${conditions.join(" AND ")} ORDER BY id ASC`,
    params,
  );
  ...
  for (const row of rows) {
    ...
    const promptTokens = Number(row.promptTokens ?? tokens.prompt_tokens ?? tokens.input_tokens) || 0;
    const completionTokens = Number(row.completionTokens ?? tokens.completion_tokens ?? tokens.output_tokens) || 0;
    const cachedTokens = Number(tokens.cached_tokens ?? tokens.cache_read_input_tokens) || 0;

    observed.requests += 1;
    observed.promptTokens += Math.max(0, promptTokens);
    observed.completionTokens += Math.max(0, completionTokens);
    observed.cachedTokens += Math.max(0, cachedTokens);
    observed.lastUsed = maxTimestamp(observed.lastUsed, row.timestamp);
    totalCost += Number(row.cost) || 0;

    if (provider === "novita" && !getPricingForModel(row.provider, row.model)) {
      allPriced = false;
    }
  }
  ...
  if (provider === "workbuddy") {
    ...
    observed.pricingStatus = "not_applicable";
  } else if (provider === "novita") {
    observed.pricingStatus = rows.length === 0 ? "no_data" : allPriced ? "priced" : "unpriced";
    observed.costUsd = rows.length === 0 || allPriced
      ? Math.round(totalCost * 1e8) / 1e8
      : null;
  } else {
    observed.costUsd = Math.round(totalCost * 1e8) / 1e8;
    observed.pricingStatus = rows.length > 0 ? "priced" : "no_data";
  }
