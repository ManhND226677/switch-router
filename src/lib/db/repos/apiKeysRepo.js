...
export async function getApiKeys(options = {}) {
  ...
  const visibilityClause = includeHidden ? "" : " WHERE COALESCE(isHidden, 0) = 0";
  const rows = db.all(`SELECT * FROM apiKeys${visibilityClause} ORDER BY createdAt ASC`);
...
export async function getApiKeyById(id, options = {}) {
  const db = await getAdapter();
  const includeHidden = options?.includeHidden !== false;
  const visibilityClause = includeHidden ? "" : " AND COALESCE(isHidden, 0) = 0";
  const row = db.get(`SELECT * FROM apiKeys WHERE id = ?${visibilityClause}`, [id]);
  return rowToKey(row);
}
 ...
 * @param {{ allowedModels?: string[], monthlyBudgetUsd?: number|null, rateLimitRpm?: number|null, expiresAt?: string|null, isHidden?: boolean }} [policy]
 */
export async function createApiKey(name, machineId, policy = {}) {
  ...
  db.run(
    `INSERT INTO apiKeys(id, key, name, machineId, isActive, isHidden, createdAt, allowedModels, monthlyBudgetUsd, rateLimitRpm, expiresAt, lastUsedAt) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [apiKey.id, apiKey.key, apiKey.name, apiKey.machineId, 1, apiKey.isHidden ? 1 : 0, apiKey.createdAt, stringifyJson(apiKey.allowedModels), apiKey.monthlyBudgetUsd, apiKey.rateLimitRpm, apiKey.expiresAt, apiKey.lastUsedAt]
  );
...
export async function updateApiKey(id, data) {
  ...
  db.transaction(() => {
    ...
    db.run(
      `UPDATE apiKeys SET key = ?, name = ?, machineId = ?, isActive = ?, isHidden = ?, allowedModels = ?, monthlyBudgetUsd = ?, rateLimitRpm = ?, expiresAt = ?, lastUsedAt = ? WHERE id = ?`,
      [merged.key, merged.name, merged.machineId, merged.isActive ? 1 : 0, merged.isHidden ? 1 : 0, stringifyJson(merged.allowedModels), merged.monthlyBudgetUsd, merged.rateLimitRpm, merged.expiresAt, merged.lastUsedAt, id]
    );
    result = merged;
  });
  invalidateApiKeysCache();
