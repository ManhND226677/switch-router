...
let conflictNoticeLogged = false;

// Deployment-level kill-switch values for OBSERVABILITY_ENABLED. Anything else
// (including unset) leaves the decision to the stored setting.
const ENV_OFF_VALUES = new Set(["false", "0", "off", "no"]);

/**
 * Resolve the stored observability flag across the key names that ever existed.
 *
 * `enableObservability` is canonical (DEFAULT_SETTINGS + the dashboard toggle).
 * `observabilityEnabled` is a legacy alias an older build wrote and that this
 * DB still carries — it may disagree with the canonical key, so it is only a
 * fallback. `enableObservability2` never had a writer; kept last for safety.
 */
function readStoredObservabilityFlag(settings) {
  if (typeof settings.enableObservability === "boolean") {
    return { value: settings.enableObservability, source: "settings.enableObservability" };
  }
  if (typeof settings.observabilityEnabled === "boolean") {
    return { value: settings.observabilityEnabled, source: "settings.observabilityEnabled (legacy alias)" };
  }
  if (typeof settings.enableObservability2 === "boolean") {
    return { value: settings.enableObservability2, source: "settings.enableObservability2 (legacy alias)" };
  }
...
async function getObservabilityConfig() {
  ...
  try {
    ...
    const envKillSwitch = typeof envRaw === "string" && ENV_OFF_VALUES.has(envRaw.trim().toLowerCase());
    const stored = readStoredObservabilityFlag(settings);

    // A DB that carries both keys is ambiguous; say which one wins instead of
    // letting the two silently disagree (this exact pair was seen in the wild:
    // observabilityEnabled=false next to enableObservability=true).
    if (
      !conflictNoticeLogged &&
      typeof settings.enableObservability === "boolean" &&
      typeof settings.observabilityEnabled === "boolean" &&
      settings.enableObservability !== settings.observabilityEnabled
    ) {
      conflictNoticeLogged = true;
      console.warn(
        `[requestDetailsRepo] settings conflict: enableObservability=${settings.enableObservability} ` +
        `but legacy observabilityEnabled=${settings.observabilityEnabled} — using enableObservability`,
      );
    }

    const enabled = envKillSwitch ? false : stored.value;
    if (!enabled && !disabledNoticeLogged) {
      disabledNoticeLogged = true;
      console.warn(
        `[requestDetailsRepo] request-detail logging is OFF (${envKillSwitch ? `OBSERVABILITY_ENABLED=${envRaw}` : `${stored.source}=false`}). ` +
        `No requestDetails rows will be written; usageHistory/usageDaily are unaffected.`,
      );
    }
    ...
    // Never fail silently: this used to flip observability OFF with no trace at
    // all, so a transient settings/DB error looked exactly like "the app stopped
    // logging". Keep the last known-good config when there is one so a single
    // failed read cannot disable request logging for the rest of the process.
    console.error("[requestDetailsRepo] getObservabilityConfig failed:", e?.message || String(e));
    if (!cachedConfig) {
      cachedConfig = {
        enabled: true,
...
export async function flushToDatabase() {
  ...
  try {
    ...
    // The failed batch was requeued above. Without a follow-up timer it sat in
    // memory until the *next* request happened to arrive — and was lost on
    // restart. Re-arm the debounce so a transient DB error is retried.
    if (!flushTimer && writeBuffer.length > 0) {
      const delay = cachedConfig?.flushIntervalMs || DEFAULT_FLUSH_INTERVAL_MS;
      flushTimer = setTimeout(() => {
        flushTimer = null;
        flushToDatabase().catch((err) => console.error("[requestDetailsRepo] retry flush err:", err));
      }, delay);
      flushTimer.unref?.();
    }
  } finally {
