...
function sanitizeSettingValues(raw) {
  ...
  // Observability has one toggle now. Older builds wrote `observabilityEnabled`
  // while DEFAULT_SETTINGS and the dashboard write `enableObservability`, so a
  // real DB can carry both with opposite values. Fold the legacy key in only
  // when the canonical one is absent, then drop it — after this the two can
  // never disagree again, and the stray value is erased on the next write.
  if (cleaned.enableObservability === undefined && typeof cleaned.observabilityEnabled === "boolean") {
    cleaned.enableObservability = cleaned.observabilityEnabled;
  }
  delete cleaned.observabilityEnabled;
