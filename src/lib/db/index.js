...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    for (const k of payload.apiKeys || []) {
      ...
      db.run(
        `INSERT OR REPLACE INTO apiKeys(id, key, name, machineId, isActive, isHidden, createdAt, allowedModels, monthlyBudgetUsd, rateLimitRpm, expiresAt, lastUsedAt) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [k.id, rawKey, k.name || null, k.machineId || null, k.isActive === false ? 0 : 1, k.isHidden === true ? 1 : 0, k.createdAt || new Date().toISOString(), stringifyJson(Array.isArray(k.allowedModels) ? k.allowedModels : []), k.monthlyBudgetUsd ?? null, k.rateLimitRpm ?? null, k.expiresAt ?? null, k.lastUsedAt ?? null]
      );
    }
