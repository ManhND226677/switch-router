                  ...
                  <div className="flex items-start gap-2 px-2 py-2 hover:bg-black/5 dark:hover:bg-white/5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {s.oauth ? (
                          <span className="px-1 py-0.5 text-xs rounded bg-amber-500/10 text-amber-600">OAuth</span>
                        ) : (
                          <span className="px-1 py-0.5 text-xs rounded bg-green-500/10 text-green-600">Authless</span>
                        )}
                        {s.toolCount > 0 && (
                      ...
                      {s.description && (
                        <p className="text-xs text-text-muted line-clamp-2 mt-0.5">{s.description}</p>
                      )}
                    </div>
                    <button
                  ...
                  {expanded && (
                    <div className="px-3 py-2 bg-surface/40 border-t border-border flex flex-col gap-2">
                      ...
                      {!isLoadingTools && cache?.error && !cache?.requiresAuth && (
                        <p className="text-xs text-red-600 bg-red-500/10 px-2 py-1 rounded">Probe failed: {cache.error}</p>
                      )}
                        ...
                        <>
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-1">
                              <button onClick={() => setAllTools(s.url, true)} className="text-xs text-primary hover:underline">All</button>
                              <span className="text-xs text-text-muted">·</span>
                              <button onClick={() => setAllTools(s.url, false)} className="text-xs text-primary hover:underline">None</button>
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto">
                            {toolKeys.map((t) => (
                              <label key={t} className="flex items-center gap-1.5 text-xs cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 px-1 rounded">
                                <input
                                  type="checkbox"
                                  checked={!!sel[t]}
                      ...
                      <button
                        ...
                        className="self-end px-2 py-1 rounded text-xs font-medium bg-primary text-white hover:bg-primary/90"
                      >
                        ✓ Confirm Add
                      </button>
                    </div>
