...
import { getCachedQuotaUsage, getQuotaFetchCacheKey } from "@/shared/services/quotaFetchCache";
...
function getQuotaPollInterval(providerConfig, resetAt, nowMs = Date.now()) {
  const resetMs = new Date(resetAt).getTime();
  if (!Number.isFinite(resetMs)) return C.tickIntervalMs;
  const remainingMs = resetMs - nowMs;
  if (remainingMs <= 5 * 60 * 1000) return 15 * 1000;
  if (remainingMs <= 30 * 60 * 1000) return C.tickIntervalMs;

  // Sliding reset windows need a regular check, but can still be polled less
  // often while far from the warm window. Stable windows are already guarded
  // by refreshAheadMs in pingConnection.
  if (providerConfig.pingWhenResetAtSlides) return 5 * 60 * 1000;
  return 5 * 60 * 1000;
}

function shouldSkipQuotaPoll(state, key, nowMs = Date.now()) {
  const nextCheckAt = state.nextQuotaCheckAt?.[key];
  return Number.isFinite(nextCheckAt) && nowMs < nextCheckAt;
}

async function pingConnection(conn, provider, providerConfig, handler, deps, state = g) {
  const key = cacheKey(provider, conn.id);
  const now = Date.now();
  state.nextQuotaCheckAt ||= {};
  ...
  if (!providerConfig.pingWhenResetAtSlides && cachedReset && now < new Date(cachedReset).getTime() - C.refreshAheadMs) return;
  if (shouldSkipQuotaPoll(state, key, now)) return;
  ...
  const usage = await getCachedQuotaUsage(
    getQuotaFetchCacheKey(connection),
    () => handler.getUsage(connection.accessToken, proxyOptions),
    { maxAgeMs: 60 * 1000 },
  );
  ...
  if (!resetAt) {
    state.nextQuotaCheckAt[key] = now + C.tickIntervalMs;
    return;
  }

  state.resetCache[key] = resetAt;
  state.nextQuotaCheckAt[key] = Date.now() + getQuotaPollInterval(providerConfig, resetAt);
