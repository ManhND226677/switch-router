...
export default function ComboDetailPage() {
  ...
  /* eslint-disable react-hooks/purity -- Date.now measures an explicit user-triggered request, not render. */
  ...
  const curlExample = examplePath
    ? `curl -X POST http://127.0.0.1:28701${examplePath} \\\n  -H "Content-Type: application/json" \\\n  -H "Authorization: Bearer ${apiKey || "YOUR_KEY"}" \\\n  -d '${JSON.stringify(exampleBody)}'`
    : "";
    ...
    <div className="flex flex-col gap-6">
          ...
          {testResult && (
            <div className="mt-3 flex flex-col gap-3">
              {testResult.latencyMs != null && (
                <span className="text-xs text-text-muted">⚡ {testResult.latencyMs}ms</span>
              )}
              {testResult.imageUrl && (
                <div>
      ...
      <Card>
        ...
        {logs.length === 0 ? (
          ...
          <pre className="text-xs font-mono bg-black/[0.03] dark:bg-white/[0.03] p-3 rounded-lg overflow-auto max-h-[400px] whitespace-pre-wrap">
            {logs.join("\n")}
          </pre>
        )}
      </Card>
