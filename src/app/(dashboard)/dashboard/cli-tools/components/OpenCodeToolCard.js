...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, initialStatus }) {
  ...
  useEffect(() => {
    if (status?.opencode?.models) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- hydrate model fields from the fetched status once
    ...
    if (isSwitchRouterModelId(status?.config?.agent?.explorer?.model)) {
      setSubagentModel(stripSwitchRouterModelPrefix(status.config.agent.explorer.model));
    }
  }, [status]);
  ...
  const saveModels = async (models) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (selectedApiKey || "sk_switch_router");
  ...
  const getConfigStatus = () => {
    ...
    const url = getSwitchRouterProvider(status.config?.provider)?.options?.baseURL || "";
    return matchKnownEndpoint(url) ? "configured" : "other";
  };

  const configStatus = getConfigStatus();
  const configuredProvider = getSwitchRouterProvider(status?.config?.provider);
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `${SWITCH_ROUTER_MODEL_PREFIX}${activeModelToShow}`,
        agent: {
          explorer: {
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {configuredProvider?.options?.baseURL && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasSwitchRouter} loading={restoring}>
