...
export default function ClaudeToolCard({
  ...
  useEffect(() => {
    if (claudeStatus?.installed && !hasInitializedModels.current) {
      ...
      if (tokenFromFile && apiKeys?.some(k => k.key === tokenFromFile)) {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- hydrate the selected key once from the file token
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      // Get key from dropdown, fallback to first key or sk_switch_router for localhost
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
                ...
                {tool.defaultModels.map((model) => (
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <div className="relative w-full min-w-0">
                      ...
                      {modelMappings[model.alias] && <button onClick={() => onModelMappingChange(model.alias, "")} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-text-muted hover:text-red-500 rounded transition-colors" title="Clear"><span className="material-symbols-outlined text-sm">close</span></button>}
                    </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasSwitchRouter} loading={restoring}>
