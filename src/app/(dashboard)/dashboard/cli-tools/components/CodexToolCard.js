...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, initialStatus }) {
  ...
  useEffect(() => {
    if (codexStatus?.config) {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- hydrate model fields from the parsed config once
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_switch_router for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# Switch-Router Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "switch-router"

[model_providers.switch-router]
name = "Switch-Router"
