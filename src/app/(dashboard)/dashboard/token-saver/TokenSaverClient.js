    ...
    const loadSettings = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (!embedded) refreshPxpipeStatus().then(runPxpipeHealth);
        }
      } catch {}
    };
    loadSettings();
  }, [embedded, patchSetting, refreshPxpipeStatus, runPxpipeHealth]);
          ...
          {!pxpipeStatus.installed ? (
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-text-muted">
                ...
                the Switch-Router data directory. May take a few minutes.
              </p>
            </div>
          ) : (
