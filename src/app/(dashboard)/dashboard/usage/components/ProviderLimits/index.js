...
function ObservedUsageCard({ observed }) {
  if (!observed || typeof observed !== "object") return null;
  const tokenTotal = (Number(observed.promptTokens) || 0) + (Number(observed.completionTokens) || 0);
  const costLabel = observed.costUsd === null || observed.costUsd === undefined
    ? (observed.pricingStatus === "not_applicable" ? "N/A · native credits" : "Not priced")
    : `$${Number(observed.costUsd).toFixed(6)}`;

  return (
    <div className="rounded-lg border border-sky-500/20 bg-sky-500/[0.04] px-2.5 py-2 text-xs">
      <div className="flex items-center justify-between gap-2">
        <span className="font-semibold text-sky-700 dark:text-sky-300">Local observed</span>
        <span className="text-text-muted">Gateway history · all time</span>
      </div>
      <div className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-1 text-text-muted sm:grid-cols-4">
        <span><strong className="text-text-primary">{Number(observed.requests) || 0}</strong> requests</span>
        <span><strong className="text-text-primary">{tokenTotal.toLocaleString()}</strong> tokens</span>
        <span><strong className="text-text-primary">{(Number(observed.cachedTokens) || 0).toLocaleString()}</strong> cached</span>
        <span><strong className="text-text-primary">{costLabel}</strong> cost</span>
      </div>
      <div className="mt-1 text-text-muted">
        Last used: <span className="text-text-primary">{formatCreditDate(observed.lastUsed)}</span>
        {observed.pricingStatus === "unpriced" && <span className="ml-1 text-amber-600 dark:text-amber-400">· some models lack verified price</span>}
      </div>
    </div>
  );
}
          ...
          return (
            ...
            >
              ...
              <div className="px-2 py-1.5">
                ...
                {(["workbuddy", "novita"].includes(conn.provider) && quota?.raw?.observed) && (
                  <div className="mt-2">
                    <ObservedUsageCard observed={quota.raw.observed} />
                  </div>
                )}
              </div>
            </Card>
