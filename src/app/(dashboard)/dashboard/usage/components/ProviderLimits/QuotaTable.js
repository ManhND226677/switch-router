                    ...
                    <div className={compact ? "space-y-1.5" : "space-y-2"}>
                      <div className={`flex items-center justify-between ${compact ? "text-xs" : "text-sm"} mb-1`}>
                        ...
                        <span className={`font-bold ${colors.text} shrink-0`}>
                          {hasPercentage ? `${quota.remaining}%` : (quota.limitLabel || "No limit")}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-xs text-text-muted mt-1">
                        <span className="truncate mr-2">
                          {quota.displayValue
                            || (Number.isFinite(Number(quota.used)) ? `${Number(quota.used).toLocaleString()} used` : (quota.message || "Not available"))}
                        </span>
                        <span className="shrink-0">
                          {quota.displayTotal
                            || (quota.unlimited || quota.percentageAvailable === false
                              ? (quota.unit && quota.unit !== "status" ? quota.unit : "")
                              : (Number(quota.total) > 0 ? `${Number(quota.total).toLocaleString()} total` : "∞"))}
                        </span>
                      </div>
                    </div>
                  </td>
