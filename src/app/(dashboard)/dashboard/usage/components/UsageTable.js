...
export default function UsageTable({
  ...
  useEffect(() => {
    try {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- hydrate expanded rows from localStorage after mount
                ...
                >
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-2">
                      <span className={`material-symbols-outlined text-lg text-text-muted transition-transform ${expanded.has(group.groupKey) ? "rotate-90" : ""}`}>
                        chevron_right
                      </span>
