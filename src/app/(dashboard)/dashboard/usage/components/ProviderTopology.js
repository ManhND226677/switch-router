...
export default function ProviderTopology({ providers = [], activeRequests = [], lastProvider = "", errorProvider = "" }) {
  ...
  // Derived state updated during render (React-recommended pattern): only runs when
  // the active set actually changes, preserving the old effect-based behavior without
  // touching refs or calling Date.now() during render.
  const [firstSeen, setFirstSeen] = useState({});
  const [now, setNow] = useState(() => Date.now());
  const [prevActive, setPrevActive] = useState(rawActiveSet);

  if (prevActive !== rawActiveSet) {
    setPrevActive(rawActiveSet);
    setFirstSeen((seen) => {
      const next = { ...seen };
      let changed = false;
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setNow(Date.now()), FE_ACTIVE_TICK_MS);
