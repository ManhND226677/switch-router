...
export default function TranslatorPage() {
  ...
  const handleLoad = async (stepId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        notifyError(data.error || "File not found");
      }
    } catch (e) {
  ...
  const handleToOpenAI = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) { notifyError(data.error); return; }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!provider || !model) {
        notifyWarning("Missing provider or model. Please run step 1 first to detect them.");
        return;
      }
      ...
      if (!res.ok) {
        ...
        notifyError(err.error || "Send failed");
        return;
      }
