...
export default function PxpipeClient() {
  ...
  /* eslint-disable react-hooks/set-state-in-effect -- refresh external Pxpipe status on mount. */
  useEffect(() => {
    refresh();
  }, [refresh]);
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <a href="/dashboard/profile#token-saver" className="text-xs text-primary underline hover:opacity-80">
      ...
      <Card className="p-4">
        ...
        {stats?.timeline?.some((d) => d.tokensSavedEst > 0) ? (
          ...
          <div className="h-32 flex items-center justify-center text-text-muted text-sm">
            No savings recorded yet — enable PXPIPE in Settings &gt; Token Saver and route a large Claude-format request.
          </div>
        )}
      </Card>
