...
import { Card, Button, Modal, CardSkeleton, OAuthModal, Toggle, Select, EditConnectionModal, NoAuthProxyCard, ConfirmModal } from "@/shared/components";
...
export default function ProviderDetailPage() {
  ...
  const [providerCatalog, setProviderCatalog] = useState({ scope: null, models: null });
  ...
  const catalogConnection = (providerId === "workbuddy" || providerId === "novita")
    ? connections.find((connection) => connection.isActive !== false)
    : null;
  const catalogScope = `${providerId}:${catalogConnection?.id || "none"}`;
  const catalogModels = providerCatalog.scope === catalogScope && Array.isArray(providerCatalog.models)
    ? providerCatalog.models
    : staticModels;
  const models = providerId === "workbuddy"
    ? filterWorkbuddyModels(catalogModels)
    : catalogModels;
  const providerAlias = getProviderAlias(providerId);

  const getModelRate = (model) => {
    if (providerId === "workbuddy") {
      const verification = model?.catalogSource !== "live" && model?.verifiedAt
        ? ` · Last verified ${model.verifiedAt}`
        : "";
      if (model?.billingMode === "free_trial" || model?.isFree === true) {
        return { label: "Free trial", title: `WorkBuddy native free-trial model${verification}` };
      }
      const multiplier = Number(model?.creditMultiplier);
      if (Number.isFinite(multiplier) && multiplier >= 0) {
        return {
          label: `Credit x${multiplier}`,
          title: `${model.source ? `Native WorkBuddy credit coefficient · ${model.source}` : "Native WorkBuddy credit coefficient"}${verification}`,
        };
      }
      return { label: "Not published", title: `WorkBuddy has not published a native coefficient for this model${verification}` };
    }
    if (providerId !== "novita") return null;

    const pricing = model?.pricing || getPricingForModel("novita", model?.id);
    if (!pricing) return { label: "Not published", title: "No verified Novita price is available for this model" };
    const format = (value) => Number(value).toLocaleString(undefined, { maximumFractionDigits: 4 });
    const verification = model?.catalogSource !== "live" && pricing.verifiedAt
      ? ` · Last verified ${pricing.verifiedAt}`
      : "";
    return {
      label: `$${format(pricing.input)} / 1M in · $${format(pricing.cached ?? pricing.input)} cache-read · $${format(pricing.output)} out`,
      title: `Novita USD per 1M tokens${verification}`,
    };
  };
  ...
  const oauthConnectionLabel =
    ...
    : "OAuth";
  const apiKeyConnectionLabel = "API Key";
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- fetch initial data on mount; state updates only after the responses resolve
    fetchConnections();
    fetchAliases();
    fetchCustomModels();
    fetchDisabledModels();
  ...
  // WorkBuddy and Novita catalogs are credential-scoped. Fetch them through
  // the connection endpoint after the account list is available; keep the
  // static registry visible while no active account/catalog is available.
  useEffect(() => {
    let cancelled = false;
    if (providerId !== "workbuddy" && providerId !== "novita") return undefined;

    const activeConnection = connections.find((connection) => connection.isActive !== false);
    if (!activeConnection) return undefined;

    fetch(`/api/providers/${activeConnection.id}/models?force=1`, { cache: "no-store" })
      .then(async (response) => ({ response, data: await response.json().catch(() => ({})) }))
      .then(({ response, data }) => {
        if (!cancelled && response.ok && Array.isArray(data.models) && data.models.length > 0) {
          setProviderCatalog({ scope: `${providerId}:${activeConnection.id}`, models: data.models });
        }
      })
      .catch(() => {});
  ...
  const handleImportVilaoModels = async () => {
    ...
    try {
      ...
      console.error("Error importing ViLao models:", error);
  ...
  const handleImportFreeModels = async () => {
    ...
    try {
      ...
      console.error("Error importing free models:", error);
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- prune selections that no longer reference an existing connection
  ...
  const handleTestModel = async (modelId) => {
    ...
    try {
      ...
      const isSlowProbe = providerId === "antigravity"
        ...
        {displayModels.map((model) => {
          ...
          return (
            <ModelRow
              ...
              caps={getCaps(`${providerId}/${model.id}`)}
              thinkingSuffix={resolveThinkingSuffix(model.id)}
              rateLabel={getModelRate(model)?.label}
              rateTitle={getModelRate(model)?.title}
            />
          );
        })}
        ...
        <div className="flex min-w-0 items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h2 className="truncate text-2xl font-semibold tracking-tight sm:text-3xl">{providerInfo.name}</h2>
