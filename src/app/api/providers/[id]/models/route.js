...
const PROVIDER_MODELS_CONFIG = {
  ...
  workbuddy: {
    customResolver: async (connection, { force = false } = {}) => {
      const proxy = await resolveConnectionProxyConfig(connection.providerSpecificData || {});
      const result = await resolveWorkbuddyModels(connection, {
        force,
        proxyOptions: {
      ...
      return {
        ...
        models: result.models?.length
          ? filterWorkbuddyModels(result.models)
          : filterWorkbuddyModels(getStaticProviderModels("workbuddy")),
      };
    },
  },
  novita: {
    customResolver: async (connection, { force = false } = {}) => {
      const proxy = await resolveConnectionProxyConfig(connection.providerSpecificData || {});
      const result = await resolveNovitaModels(connection, {
        force,
        proxyOptions: {
          connectionProxyEnabled: proxy.connectionProxyEnabled === true,
          connectionProxyUrl: proxy.connectionProxyUrl || "",
      ...
      return {
        ...result,
        models: result.models?.length ? result.models : getStaticProviderModels("novita"),
      };
    },
  },
  stepfun: {
...
export async function GET(request, { params }) {
  try {
    ...
    if (typeof config.customResolver === "function") {
      const result = await config.customResolver(connection, { force });
      if (result.error) {
        return NextResponse.json({
          error: result.error,
      ...
      return NextResponse.json({
        ...
        models: result.models,
        ...(result.warning ? { warning: result.warning } : {}),
        ...(result.catalogStatus ? { catalogStatus: result.catalogStatus } : {}),
