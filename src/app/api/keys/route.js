...
export async function GET() {
  try {
    const [keys, spendMap] = await Promise.all([
      getApiKeys({ includeHidden: false }),
...
export async function POST(request) {
  try {
    ...
    const apiKey = await createApiKey(name, machineId, {
      ...normalized.value,
      // Not exposed by the normal dashboard form. Used only when provisioning
      // an internal model-test key on this host.
