...
const MINIMAX_VOICE_ENDPOINT = "https://api.minimax.io/v1/get_voice";
...
/**
 * GET /api/media-providers/tts/minimax/voices[?voice_type=all]
...
export async function GET(request) {
  try {
    ...
    const connections = await getProviderConnections({ provider: "minimax", isActive: true });
    const apiKey = connections[0]?.apiKey;
    if (!apiKey) {
      return NextResponse.json({ error: "No minimax connection found" }, { status: 400 });
    }

    const res = await fetch(MINIMAX_VOICE_ENDPOINT, {
      method: "POST",
      headers: {
