...
const PROVIDER_NAME = "Switch-Router";
const API_KEY_ENV = "OPENAI_API_KEY";

const getHermesDir = () => path.join(getRuntimeHomeDir(), ".hermes");
...
// Detect Hermes: PATH/global-bin lookup (the agent installs as `hermes` or
// `hermes-agent`), else the ~/.hermes config/env it writes on first run.
const checkHermesInstalled = async () =>
  (await detectCli({
    commands: ["hermes", "hermes-agent"],
    markers: [getHermesConfigPath(), getHermesEnvPath(), getHermesDir()],
  })).installed;
...
// Detect Switch-Router by base_url pointing to the local loopback interface.
const hasSwitchRouterConfig = (modelCfg) => {
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      installed: true,
      settings: { model },
      hasSwitchRouter: hasSwitchRouterConfig(model),
