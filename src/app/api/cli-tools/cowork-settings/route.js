...
import { accessRuntimePath } from "@/lib/runtimePaths";

const APP_PORT = SERVER_CONFIG.appPort;
...
const getCandidateRoots = () => {
  if (os.platform() === "darwin") {
    const base = joinRuntimePath(getRuntimeHomeDir(), "Library", "Application Support");
    return [joinRuntimePath(base, "Claude-3p"), joinRuntimePath(base, "Claude")];
  }
  if (os.platform() === "win32") {
    const localApp = process.env.LOCALAPPDATA || joinRuntimePath(getRuntimeHomeDir(), "AppData", "Local");
    const roaming = process.env.APPDATA || joinRuntimePath(getRuntimeHomeDir(), "AppData", "Roaming");
    return [
  ...
  return [
    joinRuntimePath(getRuntimeHomeDir(), ".config", "Claude-3p"),
    joinRuntimePath(getRuntimeHomeDir(), ".config", "Claude"),
  ];
};

const getAppInstallPaths = () => {
  if (os.platform() === "darwin") {
    return ["/Applications/Claude.app", joinRuntimePath(getRuntimeHomeDir(), "Applications", "Claude.app")];
  }
  if (os.platform() === "win32") {
    const localApp = process.env.LOCALAPPDATA || joinRuntimePath(getRuntimeHomeDir(), "AppData", "Local");
...
const resolveAppRootForRead = async () => {
  ...
  for (const dir of candidates) {
    try {
      await accessRuntimePath(joinRuntimePath(dir, "configLibrary"));
      return dir;
    } catch { /* try next */ }
  }
  return candidates[0];
...
const get1pRoot = () => {
  if (os.platform() === "darwin") return joinRuntimePath(getRuntimeHomeDir(), "Library", "Application Support", "Claude");
  if (os.platform() === "win32") {
    const roaming = process.env.APPDATA || joinRuntimePath(getRuntimeHomeDir(), "AppData", "Roaming");
    return joinRuntimePath(roaming, "Claude");
  }
  return joinRuntimePath(getRuntimeHomeDir(), ".config", "Claude");
};

const get1pConfigPath = () => joinRuntimePath(get1pRoot(), "claude_desktop_config.json");
...
const checkInstalled = async () => {
  for (const dir of [...getCandidateRoots(), ...getAppInstallPaths()]) {
    try { await accessRuntimePath(dir); return true; } catch { /* try next */ }
  }
  return false;
};
...
async function writeSkipApprovals(managedServers) {
  const cfgPath = joinRuntimePath(getWriteRoot(), "config.json");
  let cfg = {};
...
export async function GET() {
  try {
    ...
    const hasSwitchRouter = !!(config?.inferenceProvider === PROVIDER && baseUrl);
