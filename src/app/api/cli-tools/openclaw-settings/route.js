...
// Check if settings has a Switch-Router config.
const hasSwitchRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!getSwitchRouterProvider(settings.models.providers);
};

// Read per-agent models.json and return the current model id without its provider prefix.
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = getSwitchRouterProvider(data?.providers)?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasSwitchRouter: hasSwitchRouterConfig(settings),
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  delete existing.providers[LEGACY_SWITCH_ROUTER_PROVIDER];
  existing.providers[SWITCH_ROUTER_PROVIDER] = {
    baseUrl,
...
// POST - Update Switch-Router settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `${SWITCH_ROUTER_MODEL_PREFIX}${model}`;

    // Remove all old Switch-Router and legacy 9router entries from the allowlist.
    Object.keys(settings.agents.defaults.models)
      .filter((k) => isSwitchRouterModelId(k))
    ...
    // Add fresh Switch-Router models to the allowlist.
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`${SWITCH_ROUTER_MODEL_PREFIX}${m}`] = {};
    });
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (isSwitchRouterModelId(resolveAgentModel(agent.model))) {
    ...
    // Update the canonical provider key and remove the legacy key after migration.
    delete settings.models.providers[LEGACY_SWITCH_ROUTER_PROVIDER];
    settings.models.providers[SWITCH_ROUTER_PROVIDER] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `${SWITCH_ROUTER_MODEL_PREFIX}${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove both canonical and legacy provider entries.
    if (settings.models && settings.models.providers) {
      delete settings.models.providers[SWITCH_ROUTER_PROVIDER];
      delete settings.models.providers[LEGACY_SWITCH_ROUTER_PROVIDER];
    ...
    // Remove Switch-Router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => isSwitchRouterModelId(k));
    ...
    // Reset agents.defaults.model.primary if it uses Switch-Router.
    if (isSwitchRouterModelId(settings.agents?.defaults?.model?.primary)) {
