...
export async function getInternalHeaders(connectionId = null) {
  let apiKey = null;
  try {
    const keys = await getApiKeys({ includeHidden: true });
    // Internal model probes must not inherit a user's restrictive allowlist
    // (for example MyCowork may allow only one WorkBuddy model). Prefer the
    // dedicated hidden probe key; retain the visible-key fallback for older
    // installations that have not provisioned one yet.
    apiKey = keys.find((k) => k.isActive !== false && k.isHidden === true)?.key
      || keys.find((k) => k.isActive !== false)?.key
      || null;
  } catch {}
