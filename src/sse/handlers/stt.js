...
export async function handleStt(request) {
  ...
  const modelStr = modelForProvider(formData.get("model"), selector.providerHint);
  ...
  while (true) {
    const credentials = await getProviderCredentials(provider, excludeConnectionIds, model, { preferredConnectionId: selector.preferredConnectionId });
