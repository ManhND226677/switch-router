...
import { ensureWorkbuddyAutoCheckinStarted } from "@/shared/services/workbuddyAutoCheckin.js";
...
async function handleSingleModelRequest(body, modelStr, clientRawRequest = null, request = null, apiKey = null, modelInfo = null, preferredConnectionId = null, settingsHint = null) {
  ...
  const chatSettings = settingsHint || await getSettings();
  // Any chat routed to WorkBuddy is proof the user still wants its rewards
  // claimed — keep the auto check-in scheduler alive (no-op when already up).
  if (provider === "workbuddy") ensureWorkbuddyAutoCheckinStarted();
