...
async function resolveVideoProvider(parsedBody, providerHint = null) {
  if (!parsedBody?.model) return { provider: DEFAULT_VIDEO_PROVIDER, model: null };

  const modelStr = String(modelForProvider(parsedBody.model, providerHint));
...
function withConnectionHeader(response, connectionId) {
  ...
  headers.set("x-switch-router-connection-id", String(connectionId));
  // Preserve the legacy response header for existing video clients.
  headers.set("x-9router-connection-id", String(connectionId));
  return new Response(response.body, { status: response.status, headers });
}

async function getPinnedConnection(connectionId) {
  if (!connectionId) return null;
  try {
    const localDb = await import("@/lib/localDb");
    if (typeof localDb.getProviderConnectionById !== "function") return null;
    return await localDb.getProviderConnectionById(connectionId);
  } catch {
    return null;
  }
}
...
export async function handleVideoCreate(request, action) {
  ...
  const selector = resolveConnectionSelector(request, bodyInfo.parsed || {});
  const resolved = await resolveVideoProvider(bodyInfo.parsed, selector.providerHint);
  ...
  const preferredConnectionId = selector.preferredConnectionId;
...
export async function handleVideoGet(request, requestId) {
  ...
  const preferredConnectionId = resolveConnectionSelector(request).preferredConnectionId;

  let provider = DEFAULT_VIDEO_PROVIDER;
  if (preferredConnectionId) {
    const pinnedConnection = await getPinnedConnection(preferredConnectionId);
