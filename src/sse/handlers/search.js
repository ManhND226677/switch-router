...
export async function handleSearch(request) {
  ...
  const selector = resolveConnectionSelector(request, body);
  const providerInput = selector.providerHint || body.provider || body.model;
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      body,
      models: comboModels,
      handleSingleModel: (b, m) => handleSingleProviderSearch(b, m, request, apiKey, settings, selector.preferredConnectionId),
      log,
  ...
  return handleSingleProviderSearch(body, providerInput, request, apiKey, settings, selector.preferredConnectionId);
}
